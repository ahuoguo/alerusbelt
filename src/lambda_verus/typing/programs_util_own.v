...
Section programs_util_own.
  ...
  Lemma typed_instr_of_skip_own2_own
    ...
    (ty_size ty1 = 0) →
    (ty_size tyret = 0) →
    (∀ l1 l2 x1 x2 d tid post mask iκs, ⊢ (llft_ctx -∗ time_ctx -∗ uniq_ctx -∗ elctx_interp E -∗
      llctx_interp L -∗
      invctx_interp tid mask iκs I -∗
      ty_gho ty1 x1 d (S d) tid -∗
      ty_gho ty2 x2 d (S d) tid -∗
      ⌜tr post -[(l1, x1); (l2, x2)] mask⌝ -∗
      ⧖(S (S (S d))) -∗
      £(d+1) -∗
      |={⊤}=>
  ...
  Proof.
    ...
    iIntros ([l1 x1] [l2 x2] v1 v2 d tid post mask iκs) "#LFT #TIME #UNIQ #E L I %He1 Own1 %He2 Own2 %Obs #⧖ ⧗big ⧗ £x".
    ...
    iMod (Ha l1 l2 x1 x2 d tid post mask iκs with "LFT TIME UNIQ E L I Gho1 Gho2 [//] [] £2") as "X". { iApply (persistent_time_receipt_mono with "⧖S"). lia. }
    iDestruct "X" as (xret) "[L [I [GhoRet %Obs2]]]".
    iModIntro. iExists -[(l2, xret)]. iFrame.
    iSplit; last by done. iSplit; last done. iExists v2. iExists (S (S d)).
  ...
  Lemma typed_instr_of_skip_own_own_t
    ...
    (ty_size ty1 = 0) →
    (ty_size tyret = 0) →
    (∀ l1 x1 d tid iκs post mask, ⊢ (llft_ctx -∗ time_ctx -∗ uniq_ctx -∗ elctx_interp E -∗
  ...
  Proof.
    ...
    iIntros ([l1 x1] v1 d tid post mask iκs) "#LFT #TIME #UNIQ #E L I %He1 Own1 %Obs #⧖ ⧗ ⧗' £".
    iDestruct "Own1" as "[Gho1 %Phys1]".
    destruct d as [|d]; first by done.
    iDestruct "Gho1" as "[Pt1 [Free1 Gho1]]".
    iMod (lc_fupd_elim_later with "[£] Gho1") as "Gho1". { iApply (lc_weaken with "£"). unfold advance_credits. lia. }
    iMod (Ha l1 x1 d tid iκs post mask with "LFT TIME UNIQ E L I Gho1 [//] [] ⧗") as "X". { iApply (persistent_time_receipt_mono with "⧖"). lia. }
  ...
  Proof.
    ...
    iMod (Ha l1 x1 d tid iκs post mask with "LFT TIME UNIQ E L I Gho1 [//] [] ⧗ £2") as "X". { iApply (persistent_time_receipt_mono with "⧖"). lia. }
  ...
  Lemma typed_inv_instr_of_skip_own2_emp
    ...
    (∀ l1 l2 x1 x2 d tid iκs post mask, ⊢ (llft_ctx -∗ time_ctx -∗ uniq_ctx -∗ elctx_interp E -∗
      llctx_interp L -∗ invctx_interp tid mask iκs I -∗
      ty_gho ty1 x1 d (S d) tid -∗
      ty_gho ty2 x2 d (S d) tid -∗
      ⌜tr post -[(l1, x1); (l2, x2)] mask⌝ -∗
  ...
  Proof.
    ...
    iIntros ([l1 x1] [l2 x2] v1 v2 d tid post mask iκs) "#LFT #TIME #UNIQ #E L I %He1 Own1 %He2 Own2 %Obs #⧖ ⧗big ⧗ £1".
    iDestruct "Own1" as "[Gho1 %Phys1]".
    iDestruct "Own2" as "[Gho2 %Phys2]".
    destruct d as [|d]; first by done.
    iDestruct "Gho1" as "[Pt1 [Free1 Gho1]]".
    ...
    iMod (Ha l1 l2 x1 x2 d tid iκs post mask with "LFT TIME UNIQ E L I Gho1 Gho2 [//] [] ⧗big £1") as "X". { iApply (persistent_time_receipt_mono with "⧖S"). lia. }
