...
Section typing.
  ...
  Proof. 
    ...
    split. { unfold uniq_bor, ty_gho. do 3 f_equiv.
      { unfold uniq_body.
        do 13 f_equiv.
        - destruct (SyncTy tid tid' a a0 a1) as [_ [Hg _]]. exact Hg.
        - destruct (SyncTy tid tid' a a0 a1) as [Hp _]. by rewrite Hp.
      }
      f_equiv.
  ...
  (** [uniq_resolve] removed along with [resolve] (and the borrow's
      prophecy). *)
  ...
  Proof.
    ...
    iDestruct (guards_exist_eq x0 d' g' _ (.VO[PrVar (𝔄 ↾ prval_to_inh (vπ x0)) ξi] x0 (d', g')) _ _ 0 1 1 with "[] BigGuards4") as "BigGuards5".
      { intros x'' d'' g''. iIntros "And".
        iDestruct (uniq_and_agree (PrVar (𝔄 ↾ prval_to_inh (vπ x0)) ξi) with "[And]") as "%Heq".
        { iSplit.
          - iDestruct "And" as "[A _]". iFrame "A".
          - iDestruct "And" as "[_ (? & Pc & ?)]". iFrame "Pc". }
        iPureIntro. destruct Heq as [Heq1 Heq2]. inversion Heq2. intuition. } { lia. } { lia. }
  ...
  Lemma tctx_uniq_mod_ty_out {𝔄 𝔅 ℭl} κ (f : 𝔄 →ₛ 𝔅) (fⁱ: 𝔅 →ₛ 𝔄) ty (T: tctx ℭl) p E L
    ...
    tctx_incl E L (p ◁ &uniq{κ} (<{f;fⁱ}> ty) +:: T) (p ◁ &uniq{κ} ty +:: T)
      (λ post '(bor -:: cl), λ mask, ∀ bor' ,
        uniq_bor_loc bor' = uniq_bor_loc bor →
        uniq_bor_current bor' = fⁱ ~~$ₛ (uniq_bor_current bor) →
        post (bor' -:: cl) mask).
  Proof.
    intros Alv. split.
    { intros ?? Eq  [[??]?] ?. apply forall_proper=>?. split=>???; apply Eq; auto. }
    iIntros (G tid [x y] mask post Gtimeless) "#LFT #UNIQ #E #GguardsL G /=[p T] %Obs".
    ...
    iMod (uniq_body_transform (<{f;fⁱ}> ty) ty x0 (fⁱ ~~$ₛ x0) d' g' ξi ξidx κ tid l l _ G f
        with "LFT UNIQ [] GguardsK G UniqBody") as (ζi ζidx) "[UniqBody G]". { set_solver. }
    {
...
Global Hint Resolve uniq_subtype uniq_eqtype
  : lrust_typing.
